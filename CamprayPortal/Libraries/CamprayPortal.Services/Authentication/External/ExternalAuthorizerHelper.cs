...
namespace CamprayPortal.Services.Authentication.External
{
    public static partial class ExternalAuthorizerHelper
    {
        ...
        public static void StoreParametersForRoundTrip(OpenAuthenticationParameters parameters)
        {
            ...
            session["CamprayPortal.externalauth.parameters"] = parameters;
        }
        public static OpenAuthenticationParameters RetrieveParametersFromRoundTrip(bool removeOnRetrieval)
        {
            var session = GetSession();
            var parameters = session["CamprayPortal.externalauth.parameters"];
        ...
        {
            var session = GetSession();
            session.Remove("CamprayPortal.externalauth.parameters");
        }

        public static void AddErrorsToDisplay(string error)
        {
            var session = GetSession();
            var errors = session["CamprayPortal.externalauth.errors"] as IList<string>;
            if (errors == null)
            {
                errors = new List<string>();
                session.Add("CamprayPortal.externalauth.errors", errors);
            }
            errors.Add(error);
        }
        ...
        public static IList<string> RetrieveErrorsToDisplay(bool removeOnRetrieval)
        {
            ...
            if (errors != null && removeOnRetrieval)
                session.Remove("CamprayPortal.externalauth.errors");
            return errors;
        }
    }
}